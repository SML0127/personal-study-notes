...
각 layer들은 read-only이며, docker run 명령어로 image위에 writable layer가 하나 놓여 container layer가 생성된다.(docker commit하면 container layer가 image의 image layer로 들어가 새로운 image를 형성)     
